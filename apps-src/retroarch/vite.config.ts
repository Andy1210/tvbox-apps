...
// Standalone build for the RetroArch app package. The app is served by the shell
// at /retroarch/ (same origin as /tvbox/api), so assets are referenced relatively
// (base: "./"). The build output goes straight into the host-side package at
// apps/retroarch/web/ so the registry ships the app's UI alongside its plugin.
//
// Run from the tvbox-apps root: `npm run build:retroarch`.
export default defineConfig({
  ...
  build: {
    outDir: path.resolve(__dirname, "../../apps/retroarch/web"),
    emptyOutDir: true,
  },
});
