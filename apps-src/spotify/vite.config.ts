...
// Standalone build for the Spotify app package. The app is served by the shell
// at /spotify/ (same origin as /tvbox/api), so assets are referenced relatively
// (base: "./"). The build output goes straight into the host-side package at
// apps/spotify/web/ so the registry ships the app's UI alongside its plugin.
//
// Run from the tvbox-apps root: `npm run build:spotify`.
export default defineConfig({
  ...
  build: {
    outDir: path.resolve(__dirname, "../../apps/spotify/web"),
    emptyOutDir: true,
  },
});
