...
// Standalone build for the media client app package. The app is served by the
// shell at /mediaclient/ (same origin as /tvbox/api), so assets are referenced
// relatively (base: "./"). The build output goes straight into the host-side
// package at apps/mediaclient/web/ so the registry ships the app's UI alongside
// its bridge.
//
// Run from the tvbox-apps root: `npm run build:mediaclient`.
export default defineConfig({
  ...
  build: {
    outDir: path.resolve(__dirname, "../../apps/mediaclient/web"),
    emptyOutDir: true,
  },
});
